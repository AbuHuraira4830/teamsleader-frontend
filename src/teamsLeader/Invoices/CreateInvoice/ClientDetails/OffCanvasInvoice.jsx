...
const OffCanvasInvoice = ({
  show,
  handleClose,
  handleSaveClientDetails,
  handleSelectClient,
}) => {
  const { clientDetails } = useStateContext();
  const [showClientList, setShowClientList] = useState(
    clientDetails && clientDetails.length > 0
  );

  const toggleAddForm = () => {
    setShowClientList(!showClientList); // This will switch to showing the AddInvoiceForm
  };

  useEffect(() => {
    setShowClientList(clientDetails && clientDetails.length > 0);
  }, [clientDetails]);
    ...
    >
      ...
      <Offcanvas.Body className="custom-scrollbar">
        <div className="flex justify-content-between mb-4">
          <h5 className="mt-1 me-2">New Client</h5>
          ...
          <span>
            <span>
              ...
              >
                ...
                {!showClientList ? "Show Client List" : "Add New Client"}
              </Button>
            </span>
          </span>
        </div>
        ...
        {showClientList ? (
          <InvoiceClientList handleSelectClient={handleSelectClient} />
        ) : (
          <AddInvoiceForm
...
export default OffCanvasInvoice;
